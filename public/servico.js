...
function setupPersonalInputMasks() {
  const phone = $("#customer-phone");
  const cpf = $("#customer-document");
  const applyPhone = () => {
    phone.value = formatBrazilianPhone(phone.value);
    const length = digitsOnly(phone.value).length;
    phone.setCustomValidity(!length || length === 11 ? "" : "Digite o celular completo com DDD.");
  };
  const applyCpf = () => {
    cpf.value = formatCpf(cpf.value);
    const length = digitsOnly(cpf.value).length;
    cpf.setCustomValidity(!length || length === 11 ? "" : "Digite os 11 números do CPF.");
  };
  phone.addEventListener("input", applyPhone);
  cpf.addEventListener("input", applyCpf);
  applyPhone();
  applyCpf();

  document.querySelectorAll('[name="ownerDocument"], [name="searchedDocument"]').forEach((input) => {
    input.maxLength = 18;
    input.placeholder = "000.000.000-00";
    const applyDocument = () => {
      input.value = formatCpfCnpj(input.value);
      const length = digitsOnly(input.value).length;
      input.setCustomValidity(!length || length === 11 || length === 14 ? "" : "Digite um CPF ou CNPJ completo.");
    };
    input.addEventListener("input", applyDocument);
    applyDocument();
  });
}

async function requestJson(url, options = {}, fallbackMessage = "Não foi possível concluir agora.", timeoutMs = 20000) {
  ...
  try {
    ...
    if (error?.name === "AbortError") throw new Error("A operação demorou mais que o esperado. Tente novamente.");
...
function renderPrice() {
  ...
  $("#step-five-description").textContent = priceMode === "quote" ? "Nenhuma cobrança será criada nesta etapa." : "Confira os dados antes de gerar o pagamento.";
  $("#payment-method-title").textContent = priceMode === "quote" ? "Análise do documento" : "Pagamento Pix";
  $("#payment-method-description").textContent = priceMode === "quote" ? "A equipe confirma o valor antes de disponibilizar o pagamento." : "Gere o código para concluir sua solicitação.";
...
function collectRequest() {
  ...
  data.customerPhone = digitsOnly(data.customerPhone, 11);
  ["customerDocument", "ownerDocument", "searchedDocument"].forEach((name) => {
    if (name in data) data[name] = digitsOnly(data[name], name === "customerDocument" ? 11 : 14);
  });
  return data;
}
...
function renderSummary() {
  ...
  $("#payment-message").textContent = priceMode === "quote" ? "Anexe o documento para receber o orçamento antes do pagamento." : priceCents ? "" : "O preço deste serviço precisa ser cadastrado antes de gerar o Pix.";
}
...
async function createPix(event) {
  ...
  if (priceMode === "quote") {
    ...
    try {
      ...
      }, "Não foi possível enviar o orçamento agora.");
  ...
  message.textContent = "Preparando seu Pix...";
  try {
    const result = await requestJson("/api/checkout/pix", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ serviceId: service.id, serviceName: service.name, customer: { name: data.customerName, email: data.customerEmail, phone: data.customerPhone, document: data.customerDocument }, request: data }),
    }, "Não foi possível gerar o Pix agora. Tente novamente em instantes.");
    if (!result.pixCode && !result.qrCodeImage) throw new Error("Não foi possível gerar um código Pix válido. Tente novamente.");
...
function startStatusPolling(orderId) {
  ...
  orderPolling = window.setInterval(async () => {
    try {
      const order = await requestJson(`/api/orders/${encodeURIComponent(orderId)}`, {}, "Status temporariamente indisponível.", 6000);
