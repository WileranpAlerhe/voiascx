...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(`https://justicaabertaapi.cnj.jus.br/v1/api/cidades/listar/${uf}`, { cf: { cacheTtl: 86400 } } as RequestInit);
    if (!response.ok) throw new Error("CNJ indisponível");
    const data = await response.json() as Array<{ id: number; nome: string }>;
    return NextResponse.json(data.map(({ id, nome }) => ({ id, nome })), { headers: { "Cache-Control": "public, max-age=86400" } });
  } catch {
