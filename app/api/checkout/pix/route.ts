...
export async function POST(request: Request) {
  ...
  if (!amountCents || price.mode === "quote") return NextResponse.json({ error: "Este serviço precisa de orçamento antes do pagamento." }, { status: 409 });
  if (!current.PINPAY_API_KEY) return NextResponse.json({ error: "O pagamento está temporariamente indisponível. Tente novamente em instantes." }, { status: 503 });
  ...
  await current.DB.prepare(`INSERT INTO orders (id, service_id, service_name, amount_cents, status, customer_email, payload_ciphertext, payload_iv, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(orderId, serviceId, clean(body.serviceName), amountCents, "creating_payment", customerEmail, encrypted.ciphertext, encrypted.iv, now, now).run();
  ...
  const pinpay = await fetch(`${current.PINPAY_API_BASE_URL || "https://api.usepinpay.com"}/v1/charges`, {
    method: "POST",
    headers: { Authorization: `Bearer ${current.PINPAY_API_KEY}`, "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" },
    body: payload,
  });
  const result = await pinpay.json().catch(() => ({})) as Record<string, unknown>;
  if (!pinpay.ok) {
    await current.DB.prepare("UPDATE orders SET status = ?, updated_at = ? WHERE id = ?").bind("payment_error", new Date().toISOString(), orderId).run();
  ...
  await current.DB.prepare("UPDATE orders SET status = ?, provider_charge_id = ?, updated_at = ? WHERE id = ?")
    .bind(status, providerId || null, new Date().toISOString(), orderId).run();
